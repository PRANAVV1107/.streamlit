...
e_data = conn.read(worksheet="Sheet3", use_cols=list(range(9)), ttl=5)
...
# List of field names
fields = ["Number of Masons" ,	"Number of Carpenters" ,"Number of Female Helpers","Number of Male Helpers","Number of Bar Benders","Number of Painters","Number of Electricians","Number of Tile Layers","Number of Brick Layers","Number of Granite Layers","Number of Plumbers",	"Other Labourers - specify type and number",	"NMR - MC - Numbers",	"NMR - FC - Numbers",	"NMR Mason"]

timestamp = datetime.now()

with st.form(key="Labour Details", clear_on_submit=True):

    submit_button = st.form_submit_button(label="Submit")
    st.title("Labour Details")
    ...
    if submit_button:
        if not project_name or not supervisor_name or not field_values:
            ...
            conn.update(worksheet="Sheet3", data = df)
